...
**A correction.** This was first written up as "account opening for a brand-new adult has never had a real email through it". That was wrong, and checking rather than assuming is what showed it. The audit log has two `account_opened` events with `actor_type = 'customer'` — `ACC-A33F615225` on 2026-09-15 12:43 and `ACC-1D1BE88F6D` at 19:52 — each with a parsed date of birth and an account created in the same minute as the profile. That is the adult new-sender branch's exact signature, and nothing else produces it: a manually seeded profile has a null date of birth, as the one seeded by hand in an earlier session does.

The adult branch has run, by real email, twice. What is true is narrower and worth keeping: `Extract New Sender Intent` was rewritten in this session (quote stripping, the guardian filter, the labelled date-of-birth fallback), and while that node was re-verified today through the minor path, the adult branch specifically has not run since the change. The change only alters *which text* is parsed, not what the adult branch then does with it — but that is reasoning, not evidence, and it is recorded as such.
