-- 010: A loan that needs a human decision now raises an ops_approvals row itself,
-- so the RPC response carries everything the ops alert email needs (ref code,
-- applicant name/email, amounts) and n8n never has to stitch it together.
-- Also blocks a minor account holder from taking on debt.

CREATE OR REPLACE FUNCTION public.apply_for_loan(
    p_profile_id uuid, p_account_id uuid, p_amount bigint, p_term_months integer DEFAULT 12
) RETURNS jsonb
...
BEGIN
    ...
    -- A minor cannot take on debt; the guardian must apply in their own name.
    IF EXISTS (
        SELECT 1 FROM public.account_holders
        WHERE account_id = p_account_id AND profile_id = p_profile_id AND holder_type = 'minor'
    ) THEN
        RETURN jsonb_build_object('success', false, 'error',
            'A minor account holder cannot take out a loan. A guardian must apply in their own name.');
    END IF;
    ...
    IF p_amount <= v_auto_approve_ceiling THEN
        ...
        RETURN v_disbursement || jsonb_build_object(
            ...
            'applicant_name', v_profile.full_name, 'applicant_email', v_profile.email
        );
    END IF;

    v_ops := public.create_ops_approval(
        'loan', v_loan_id,
        'Loan approval needed - Rs ' || to_char(p_amount / 100.0, 'FM999,999,999.00'),
        COALESCE(v_profile.full_name, 'A customer') || ' (' || COALESCE(v_profile.email, 'unknown') ||
        ') applied for a loan of Rs ' || to_char(p_amount / 100.0, 'FM999,999,999.00') ||
        ' over ' || p_term_months || ' month(s). Total repayable Rs ' ||
        to_char(v_total_repayable / 100.0, 'FM999,999,999.00') || ' at ' || v_interest_rate ||
        '% flat. This is above the Rs 200,000 auto-approval ceiling.',
        jsonb_build_object(
            'loan_id', v_loan_id, 'account_id', p_account_id,
            'principal_amount', p_amount, 'total_repayable', v_total_repayable,
            'monthly_payment_amount', v_monthly_payment, 'term_months', p_term_months,
            'applicant_name', v_profile.full_name, 'applicant_email', v_profile.email
        ),
...
ALTER FUNCTION public.apply_for_loan(uuid, uuid, bigint, integer) OWNER TO banking_functions;
REVOKE ALL ON FUNCTION public.apply_for_loan(uuid, uuid, bigint, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.apply_for_loan(uuid, uuid, bigint, integer) TO service_role;
