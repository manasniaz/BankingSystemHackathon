-- 011: audit_log.actor_type has no 'operator' value (the CHECK allows
-- customer | system | n8n | python | admin | trigger), so every successful
-- resolve_ops_approval() call in 009 aborted at the audit write and rolled the
-- whole decision back. Ops decisions are 'admin'.
--
-- Caught by the first live approve, which failed with
--   new row for relation "audit_log" violates check constraint
--   "audit_log_actor_type_check"
-- The transaction rolled back cleanly, so no half-applied loan was left behind.
